...
/** * DATA NORMALIZER 
 * Fixes the "Unnamed" and "0 Tickets" issue by mapping recovered data keys.
 */
function normalizeUser(userData) {
    if (!userData) return null;
    return {
        uid: userData.uid,
        displayName: userData.name || userData.displayName || 'Unnamed User',
        role: (userData.role || 'technician').toLowerCase(),
        accessibleStores: userData.stores === 'all' ? { "ALL": true } : (userData.accessibleStores || {}),
...
async function submitNewJob() {
    ...
    try {
        ...
        var photoUrls = [];
        for (var i = 0; i < _anjPhotos.length; i++) photoUrls.push(await fileToBase64(_anjPhotos[i].file));
...
async function saveTicketEdits(key) {
    ...
    try {
        ...
        for (const f of editPhotos) {
            photoUrls.push(await fileToBase64(f));
        }
        const t = currentTickets.find(x => x._key === key);
